  ...
  return (
    <ol className="flex items-center justify-center w-full p-2 space-x-2 text-sm font-normal text-center bg-white rounded-sm dark:text-gray-400 sm:text-sm dark:bg-gray-800 dark:border-gray-700 sm:p-3 sm:space-x-4 rtl:space-x-reverse">
      {steps.map((step, index) => (
        ...
        >
          <span className={`flex items-center justify-center w-7 h-7 me-2 text-sm text-gray-900 border border-gray-300 bg-gray-100 rounded-full shrink-0 ${step.number < activeStep ? "text-white border-gray-900 bg-gray-900" : ""} ${step.number === activeStep ? "border-gray-900" : ""}`}>
            {step.number}
          </span>
          {step.title}
